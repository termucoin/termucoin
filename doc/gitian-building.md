...
```bash
gpg --detach-sign ${VERSION}-linux/${SIGNER}/termubit-linux-build.assert
gpg --detach-sign ${VERSION}-win/${SIGNER}/termubit-win-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/termubit-osx-build.assert
```
...
Gitian signatures for each release are added to https://github.com/termubit/gitian.sigs.

`gitian-build.sh` will create signatures inside `gitian-output/sigs/` folder. Create a pull request to [termubit/gitian.sigs](https://github.com/termubit/gitian.sigs) to publish your signatures, the `.assert` and `.assert.sig` files.
