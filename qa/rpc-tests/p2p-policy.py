...
class P2PPolicyTests(BitcoinTestFramework):
    ...
    def setup_network(self):
        ...
        # a Termubit Core node that behaves similar to mainnet policies
    ...
    def run_test(self):
        ...
        # create a bunch of UTXO with seed money from the Termubit Core wallet
        for i in range(20):
    ...
    # spend seed money with a key not in the Termubit Core wallet.
    def spend_utxo(self, output, expected_size, retries=0):
        # construct the transaction using Termubit Core raw tx APIs
        ...
        # and send the tx from there rather than from Termubit Core, to test
