...
This tutorial will help you to go through the basics to use Termubit Core after you completed the [installation instructions](/INSTALL.md). You now have `termubitd` or `termubit-qt` executables available to run a node, and `termubit-cli`/`termubit-tx` tools to help you transact DOGE.
...
1. [Starting a termubit node](#starting-a-termubit-node)
...
Your node is now running and starts with a *synchronization process* that downloads the entire blockchain from other nodes. This operation will take many hours to complete, but you are now part of the Termubit network!

> **Note:** The rest of this guide assumes the use of a headless node. The RPC server is not exposed with `termubit-qt` until you activate the `-server` option as a startup argument, but inside the GUI application, you can use all the commands explored below (without `termubit-cli`) by going to `Help -> Debug window` and inside the popup window selecting the tab `Console`.

## Introduction to the JSON-RPC API

Termubit Core exposes a JSON-RPC interface that allows you to request information about the network, blockchain and individual transactions, send transactions to the networks and manage your wallet.

The Termubit Core installation provides the `termubit-cli` tool to interact with the JSON-RPC from the command line, and the interface is exposed over HTTP on port `8598`, so that other tools and libraries can interact with it.
...
By default, the Termubit Core software will automatically create an address for you and securely store the private key in the wallet file.
...
> **Tip:** The transaction returned is encoded in hexadecimal encoding. You can use `termubit-cli decoderawtransaction` or `termubit-tx -json` to convert the content to JSON format.

##### signrawtransaction

Before sending a transaction, it must be signed by the private key that the address was derived from. Termubit Core will automatically use the correct private key when spending UTXO known to the wallet.
...
> **Note:** To be able to query transactions not related to your own wallet, like in this example, you will need to enable the `-txindex` option. This options requires the Termubit Core software to re-index the entire blockchain, and can take up to several hours.
...
Using `termubitd -help` will display all available configuration parameters that can be added as arguments:
...
maxconnections=150
```
You can see a more concrete example [here](/contrib/debian/examples/termubit.conf).

### Mainnet, testnet and regtest

When trying out new things, for example to test your application that interacts with the Termubit chain, it is recommended to not use the main Termubit network. Multiple networks are built-in for this purpose.
...
When not specifying any network, *Mainnet* is the network used by default. To enable *testnet*, use the `termubitd -testnet`.

To enable *regtest*, use the `-regtest` option.

> **Tip:** Remember to specify the network when you want to use `termubit-cli`.

### Data directory

The data directory is the location where Termubit Core files are stored, including the wallet, log files and blocks. You can modify the location with the `-datadir` setting.
...
Running Termubit Core can require a lot of memory, so in some situations it may be necessary to optimize its usage. You can find more information about reducing the memory footprint in the [related guide](reduce-memory.md).

### Troubleshooting

By default, Termubit Core keeps detailed logs in the `debug.log` file, located in the `datadir`. Alternatively, the `-printtoconsole` parameter displays the log interactively to the terminal instead.
