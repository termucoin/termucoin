...
Several important updates have been added to Termubit Core, including fixes
backported from Bitcoin Core's upstream.

### Reduce Peer INV Object Counts

Previously, Termubit Core nodes would maintain a large list of inventory for
...
A new transaction request tracking system improves the way Termubit Core
...
In previous versions of Termubit Core, orphan transactions were processed in a
...
Termubit Core now sends a getheaders message for one block when announced in an
...
vulnerability or security issue in Termubit Core.
...
* Termubit Core now supports compilation with C++14 and C++17, if you select
